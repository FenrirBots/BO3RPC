//https://github.com/zzfeed/HWBP-Hooking
...
LONG __stdcall _internal_handler(PEXCEPTION_POINTERS info) {
	...
	if(info->ContextRecord->XIP == (reinterpret_cast<unsigned long long>(GetModuleHandleA(0)) + (0x7FF77343C415 - 0x7FF771910000)))
	{
		if(g_entities == 0)
		{
			g_entities = (unsigned long long)(info->ContextRecord->Rax);
		}
...
bool hwbp_hook::hook(void* target, void* hook, HANDLE thread) {
	...
	if (thread == nullptr) {
		for (auto t : *_manager->threads()) {
			set_hook(t, _original);
		}
	}
	else
...
bool hwbp_hook::unhook(HANDLE thread) {
	...
	if (thread != nullptr) {
		...
		for (auto t : *_manager->threads()) {
			...
			if (t == INVALID_HANDLE_VALUE || t == nullptr)
				continue;
